...
```
In th point plot, I have tried to visualize the count for Miss Distance for a give n date range with diameter as a size and if that particular asteroid is hazardous or not. Few key finding from the plots:  
...
The dark line in box plot represent the median and the top box is 75%ile and bottom box is 25 %ile. The end points of the black line are whiskers and they are at a distance of 1.5*IQR. In this plot we can visualize that the median diameter of potentially hazardous asteroid is hgher than the non hazardous asteroid. From the plot, we can also visualize few extreme points (>1.5IQR).  
...
Lastly, I took the dates (time-stamp) of events, and extracted months and years from them, and plotted a histogram the count of CME events occurring in each month, and in each year between the given input start and end dates, classified into the type of events on the histograms bins.
